...
p.addParameter('plotStop',true,@islogical);
p.addParameter('plotRotationCenters',true,@islogical);
p.addParameter('plotVisualAxis',false,@islogical);
p.addParameter('plotFovea',true,@islogical);
...
if p.Results.newFigure
    figHandle = figure();
    figHandle.Position = [100 100 600 600];
end
hold on
...
if p.Results.plotStop
    A = [eye.stop.center(PdimA)+2*sind(eye.stop.thetas(2)), -2; eye.stop.center(PdimA)-2*sind(eye.stop.thetas(2)), 2];
    plot(A(:,1),A(:,2),['-' p.Results.plotColor]);
end

if p.Results.plotRotationCenters
    plot(eye.rotationCenters.azi(PdimA),eye.rotationCenters.azi(PdimB),['*' p.Results.plotColor], 'MarkerSize',12)
    plot(eye.rotationCenters.ele(PdimA),eye.rotationCenters.ele(PdimB),['o' p.Results.plotColor], 'MarkerSize',12)
end
...
if isfield(eye,'landmarks')
    ...
    if isfield(eye.landmarks,'fovea')
        ...
        plot(eye.landmarks.fovea.coords(PdimA),eye.landmarks.fovea.coords(PdimB),['s' p.Results.plotColor], 'MarkerSize',12)
        end        
...
%% Reference axis
% xRange = xlim;
plot([-25, 1],[0 0],['--' p.Results.plotColor]);
