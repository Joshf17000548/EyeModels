function [outputRay, initialRay, targetIntersectError, glintPath] = findGlintRay( worldOrigin, eyePose, worldTarget, rotationCenters, opticalSystemFixRL, opticalSystemRot, opticalSystemFixLR, cameraRotation)
...
intersectErrorTolerance = 3e-4;
searchIterTolerance = 100;
searchIter = 0;
searchingFlag = true;
bestError = Inf;
bestAngles = [nan nan];
...
% plot3(cornealApex(:,1), cornealApex(:,2), cornealApex(:,3), 'x')
...
% the corneal apex.
% plot3(cornealApex(:,1), cornealApex(:,2), cornealApex(:,3), 'o')

[angle_p1p2, angle_p1p3] = quadric.rayToAngles(quadric.normalizeRay([eyeCoordOrigin; cornealApexRotated-[8 0 0] - eyeCoordOrigin]'));
% cc= cornealApexRotated
% plot3(cornealApex(1), cornealApex(2), cornealApex(3), 's' )
...
% plot3(cornealApex(1), bb(3), bb(5), 'o' )
% plot3(cornealApex(1), bb(3), bb(6), 'o' )
% plot3(cornealApex(1), bb(4), bb(5), 'o' )
% plot3(cornealApex(1), bb(4), bb(6), 'o' )
...
% plot3(cornealApexRotated(1), bb(3), bb(5), 'x' )
% plot3(cornealApexRotated(1), bb(3), bb(6), 'x' )
% plot3(cornealApexRotated(1), bb(4), bb(5), 'x' )
% plot3(cornealApexRotated(1), bb(4), bb(6), 'x' )
...
for xx=1:2
    for yy=1:2
        cornerPoint = [cornealApexRotated(1),bb(2+xx),bb(4+yy)];
        [p1p2B,p1p3B] = ...
...
intersectErrorFunc = @(p1p2,p1p3) calcTargetIntersectError(eyeCoordOrigin, wrapTo180(p1p2+p1p2Adjust), wrapTo180(p1p3+p1p3Adjust), eyePose, worldTarget, rotationCenters, opticalSystemFixRL, opticalSystemRot, opticalSystemFixLR, Rstruc, cameraRotation);
...
% t = [inputRayEyeCoord(:,1)'; (inputRayEyeCoord(:,1)-700.*inputRayEyeCoord(:,2))'];
% plot3(t(:,1), t(:,2), t(:,3), '-')
...
%         [angle_p1p2,err]=fminsearch(myObj_p1p2,angle_p1p2,options);      
...
% t = [initialRay(1,:); (initialRay(1,:)+700.*initialRay(2,:))]
% plot3(t(:,1), t(:,2), t(:,3), '-')
[outputRay, glintPath] = threeSystemTrace(initialRay', eyePose, rotationCenters, opticalSystemFixRL, opticalSystemRot, opticalSystemFixLR, Rstruc);
...
function distance = calcTargetIntersectError(eyeCoordOrigin, angle_p1p2, angle_p1p3, eyePose, worldTarget, rotationCenters, opticalSystemFixRL, opticalSystemRot, opticalSystemFixLR, Rstruc, cameraRotation)
...
n = quadric.anglesToRay(eyeCoordTarget', cameraRotation(2), -cameraRotation(1) );
% t = [n(:,1), (n(:,1)+700.*n(:,2))];
% plot3(t(1,:), t(2,:), t(3,:), '-')
inter = quadric.intersectRayPlane(n(:,2), eyeCoordTarget',outputRayEyeCoord');
%  plot3(inter(1), inter(2), inter(3), 'o')
if(outputRayEyeCoord(2,1)<0)
    distance = 1000;
else
    distance = norm(inter-eyeCoordTarget');
end

end % calcTargetIntersectError


function [outputRayEyeCoord, glintPath] = threeSystemTrace(inputRayEyeCoord, eyePose, rotationCenters, opticalSystemFixRL, opticalSystemRot, opticalSystemFixLR, Rstruc)

% Ray trace through the fixed system (typically, camera to the medium
% adjacent to the eye)
% plot3(inputRayEyeCoord(1,1), inputRayEyeCoord(2,1), inputRayEyeCoord(3,1))
% t = [inputRayEyeCoord(:,1)'; (inputRayEyeCoord(:,1)+700.*inputRayEyeCoord(:,2))'];
% plot3(t(:,1), t(:,2), t(:,3), '-')
...
% plot3(outputRayEyeCoord(1,1), outputRayEyeCoord(1,2), outputRayEyeCoord(1,3))
% t = [outputRayEyeCoord(1,:); (outputRayEyeCoord(1,:)+700.*outputRayEyeCoord(2,:))];
% plot3(t(:,1), t(:,2), t(:,3), '-')
...
[outputRayEyeCoord, glintPath]= rayTraceQuadrics(outputRayEyeCoord', opticalSystemFixLR);
