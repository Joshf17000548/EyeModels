function [eyePose,errors, desiredFixationPoint] = calcFixationPose(eye,desiredFixationPoint,stopRadius,addPseudoTorsionFlag,cameraMedium)
...
arguments
    eye (1,1) {isstruct}
    desiredFixationPoint (3,1) {mustBeNumeric} = [50, 50, 50]
...
desiredFixationPoint = desiredFixationPoint([3,1,2],:);
incidentNodeEye = eye.landmarks.incidentNode.coords';


% Obtain the coordinates of the fovea
rayDestination = eye.landmarks.fovea.coords';
targetDistance = norm(desiredFixationPoint' - incidentNodeEye);
...
% desiredFixationPoint = convertWorldToEyeCoord(desiredFixationPoint);
...
% solution.
R1 = quadric.normalizeRay([incidentNodeEye, [1;0;0]]);
R2 = quadric.normalizeRay([incidentNodeEye, desiredFixationPoint - incidentNodeEye]);
[~, angle_xy, angle_xz] = quadric.angleRays( R1, R2 );

p0 = [angle_xy, angle_xz] - eye.landmarks.fovea.degField;
