function [pupilEllipse, glintCoord, imagePoints, worldPoints, headPoints, eyePoints, cameraPoints, pointLabels, targetIntersectError, pupilFitError, rayPath, glintPath, error] = projectModelEye(eyePose, sceneGeometry, varargin)
...
p.addParameter('nStopPerimPoints',30,@(x)(isscalar(x) && x>4));
...
p.addParameter('pupilRayFunc',@findPupilRay,@(x)(isa(x,'function_handle') || isempty(x)));
p.addParameter('glintRayFunc',@findGlintRay,@(x)(isa(x,'function_handle') || isempty(x)));
...
[eyePoints, pointLabels, targetIntersectError, rayPath, error_pupil] = ...
    refractEyePoints(eyePoints,pointLabels,sceneGeometry,p,eyePose);

%% Add glint(s)
[eyePoints, pointLabels, glintPath, error_glint] = ...
    addGlint(eyePoints,pointLabels,sceneGeometry,p,eyePose);

error = [error_pupil, error_glint];
...
[imagePointsPreDistortion, cameraPoints] = ...
    projectToImagePlane(worldPoints,sceneGeometry);

% plot3(eyePoints(:,1),eyePoints(:,2), eyePoints(:,3), 'o')
% plot3(headPoints(:,1),headPoints(:,2), headPoints(:,3), 'o')
% 
% % figure(1)
% w = convertWorldToEyeCoord(sceneGeometry.cameraPosition.translation);
% n = (w - headPoints(31,:))./norm(w - headPoints(31,:));
% ray = [headPoints(31,:); headPoints(31,:) + n*650];
% plot3(ray(:,1),ray(:,2), ray(:,3), '-')
% plot3(headPoints(:,1),headPoints(:,2), headPoints(:,3), 'o')
% % plot3(cameraPoints(:,1),cameraPoints(:,2), cameraPoints(:,3), 'o')
% xlim([-25,5])
% ylim([-15,15])
% zlim([-25,25])
