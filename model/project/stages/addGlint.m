function [eyePoints, pointLabels, glintPath, error] = addGlint(eyePoints,pointLabels,sceneGeometry,p,eyePose)
...
args = {sceneGeometry.cameraPosition.translation, ...
    ...
    sceneGeometry.cameraPosition.rotation};

glintPath = cell(1,length(nGlints));
% Loop through the glints

error = zeros(length(nGlints),1);

for gg = 1:nGlints

    % Perform the computation using the passed function handle
    [virtualImageRay, ~, intersectError, glintPath{gg}] = ...
        glintRayFunc(glintSourceWorld(:, gg), eyePose, args{:});

    error(gg) = ~(intersectError < rayTraceErrorThreshold);
