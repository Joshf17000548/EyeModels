...
S = quadric.rotate(S, [0, 1.241, 2.983]);
S = quadric.translate(S,[-radii(1) 0.222 -0.225]);
...
% radii = [6.5, 6.5, 6.5];
radii = [6.5728  6.4467  6.4467];
S = quadric.scale(quadric.unitSphere,radii);

S = quadric.rotate(S,[0, 1.241, 2.983]);

S = quadric.translate(S,[delta 0 0]);
S = quadric.translate(S,[-radii(1) 0.194 -0.213]);

cornea.back.S = quadric.matrixToVec(S);
cornea.back.side = 1;
cornea.back.boundingBox=[-4 X(1)-delta -9 9 -9 9];
