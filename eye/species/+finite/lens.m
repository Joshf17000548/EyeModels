...
S = quadric.rotate(S,[0 0.91 -1.643]);
S = quadric.translate(S,[lens.back(1)-lensBackRadii(1) -0.27 0.092]);

% A bounding box for the back lens surface
boundingBox = [lens.back(1)-1 lens.center(1) -5 5 -5 5];
...
for ii = 1:numShells
    ...
    S = quadric.rotate(S,[0 0.91 -1.643]);
    decentration = [-0.27 - 0.101*((1-proportion)/2), 0.092 - 0.056*((1-proportion)/2)];
    S = quadric.translate(S,[lens.center(1)-(proportion*lensThickBack)-radius decentration(1) decentration(2)]);
...
% S = quadric.scale(frontQuadricPrimitive,lensFrontRadii(1));
S = quadric.rotate(S,[0 0.91, -1.643]);
S = quadric.translate(S,[lens.front(1)-lensFrontRadii(1) -0.371 0.036]);
...
for ii = numShells:-1:1
    ...
    % Position the surface 
    S = quadric.rotate(S,[0 0.91, -1.643]);
    decentration = [-0.371 + 0.101*((1-proportion)/2), 0.036 + 0.056*((1-proportion)/2)];
    S = quadric.translate(S,[lens.center(1)+(proportion*lensThickFront)+radius decentration(1) decentration(2)]);
...
boundingBox = [lens.center(1) lens.front(1)+1 -5 5 -5 5];
lens.S = [lens.S; quadric.matrixToVec(S)];
lens.boundingBox = [lens.boundingBox; boundingBox];
lens.side = [lens.side; 1];
lens.mustIntersect = [lens.mustIntersect; 1];
